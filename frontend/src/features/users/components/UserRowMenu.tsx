...
function MenuItem({
...
}: { children: React.ReactNode; onClick: () => void; tone: 'ink' | 'ok' | 'warn' | 'danger' }) {
  const color =
    tone === 'ok' ? 'var(--accent)' :
    tone === 'warn' ? 'var(--warn)' :
  ...
  return (
    <button
      ...
      className="users-menu-item w-full text-left transition-colors"
      style={{
    ...
    >
      {children}
      <style>{`.users-menu-item:hover { background: var(--surface-mute); }`}</style>
    </button>
  )
}
